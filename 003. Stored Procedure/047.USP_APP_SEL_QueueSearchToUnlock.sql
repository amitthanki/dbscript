IF OBJECT_ID ('USP_APP_SEL_QueueSearchToUnlock') IS NOT NULL 
DROP PROC [DBO].[USP_APP_SEL_QueueSearchToUnlock]
GO

/****** Object:  StoredProcedure [dbo].[USP_APP_SEL_QueueSearchToUnlock]    Script Date: 05/07/2017 ******/
SET ANSI_NULLS ON
GO
...
-- Description: Get Locked Records(OST,Eligibility, RPR) to unlock
-- Eg.
 --Declare @ErrorMessage VARCHAR(2000)
 --Set @ErrorMessage=''
 --Exec [USP_APP_SEL_QueueSearchToUnlock] ,@WorkItemId=1,@ErrorMessage = @ErrorMessage Output
-- =============================================
Create PROCEDURE [dbo].[USP_APP_SEL_QueueSearchToUnlock]
...
Select DISTINCT TOP 500 
	...
	JOIN ADM_UserMaster UMLocked with(nolock) ON GQ.LockedByRef=UMLocked.ADM_UserMasterId
	JOIN GEN_OSTActions GOST with(nolock) ON GQ.GEN_QueueId=GOST.GEN_QueueRef
	JOIN CMN_RecordsLocked RL with(nolock) ON RL.CaseId=GQ.GEN_QueueId
	LEFT JOIN ADM_UserMaster UMLastUpdate with(nolock) ON GQ.LastUpdatedByRef=UMLastUpdate.ADM_UserMasterId
	LEFT JOIN ADM_UserMaster UMAssigned with(nolock) ON GQ.AssignedToRef=UMAssigned.ADM_UserMasterId
	...
	AND GQ.IsActive=1
	AND GQ.UTCLockedOn is not null
	AND RL.IsActive=1
