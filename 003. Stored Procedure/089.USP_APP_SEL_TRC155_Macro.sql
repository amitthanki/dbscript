IF OBJECT_ID ('USP_APP_SEL_TRC155_Macro') IS NOT NULL 
DROP PROC [DBO].[USP_APP_SEL_TRC155_Macro]
GO

/****** Object:  StoredProcedure [dbo].[USP_APP_SEL_TRC155_Macro]    Script Date: 01/29/2018 ******/
SET ANSI_NULLS ON
GO
...
-- Description: Get TRC 155 Status the queue for Macro
-- Eg.
-- Declare @ErrorMessage VARCHAR(2000)
-- Set @ErrorMessage=''
-- Exec [USP_APP_SEL_TRC155_Macro] @ErrorMessage = @ErrorMessage Output
-- =============================================
CREATE PROCEDURE [dbo].[USP_APP_SEL_TRC155_Macro]
    ...
    select @DiscrepancyTypeLkup = CMN_LookupMasterID  from CMN_LookupMaster with(nolock) where CMN_LookupTypeRef = 7 And LookupValue = @DiscrepancyType;
                     ...
                     Select GEN_QueueId as WorkItemID, 
                      ...
                      Where                                           
                            ...
                            AND (GQ.MostRecentWorkQueueLkup = 10047)  ---TRR - TRC 155 
							AND (GQ.IsTRC155MacroProcessed = 0 or GQ.IsTRC155MacroProcessed Is NULL)
END TRY 
