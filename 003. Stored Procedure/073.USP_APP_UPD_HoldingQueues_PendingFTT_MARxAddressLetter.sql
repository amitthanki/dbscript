IF OBJECT_ID ('USP_APP_UPD_HoldingQueues_PendingFTT_MARxAddressLetter') IS NOT NULL 
BEGIN
	DROP PROC [dbo].[USP_APP_UPD_HoldingQueues_PendingFTT_MARxAddressLetter]
END
GO
...
-- Eg.
-- Exec [USP_APP_UPD_HoldingQueues_PendingFTT_MARxAddressLetter]
-- =============================================
CREATE PROC [dbo].[USP_APP_UPD_HoldingQueues_PendingFTT_MARxAddressLetter]
as
BEGIN	
	...
	BEGIN TRY  
		BEGIN TRAN PendingFTT_MARxAddressLetter
		...
		WHERE GQ.[MostRecentWorkQueueLkup] = 10012 AND
			  ...
			  (GQ.[IsMARxAddressCompleted] IS NULL OR GQ.[IsMARxAddressCompleted] = 0) AND (GQ.TRCLkup <> 22006) AND
		...
		WHILE(1 = 1)
			BEGIN
				...
				SET @ActionLkup = 28052 --Requires MARx Address Letter
				...
				SET [ActionLkup]=@ActionLkup,[IsActive]=@IsActive,[PendReasonLkup]=30021,--Pend Reason - MARx Address Letter
				...
				-----------Unlock record END-----------
			END
		COMMIT TRAN PendingFTT_MARxAddressLetter
	END TRY
	BEGIN CATCH		
			...
			VALUES(@LoginUserId,'[dbo].[USP_APP_UPD_HoldingQueues_PendingFTT_MARxAddressLetter]',null,ERROR_NUMBER(),ERROR_MESSAGE(),GETUTCDATE(),GETUTCDATE())
