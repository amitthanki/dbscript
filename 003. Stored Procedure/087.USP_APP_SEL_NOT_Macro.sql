IF OBJECT_ID ('USP_APP_SEL_NOT_Macro') IS NOT NULL 
DROP PROC [DBO].[USP_APP_SEL_NOT_Macro]
GO

/****** Object:  StoredProcedure [dbo].[USP_APP_SEL_NOT_Macro]    Script Date: 01/29/2018 ******/
SET ANSI_NULLS ON
GO
...
-- Description: Get Open Not Status the queue for Macro
-- Eg.
-- Declare @ErrorMessage VARCHAR(2000)
-- Set @ErrorMessage=''
-- Exec [USP_APP_SEL_NOT_Macro] @ErrorMessage = @ErrorMessage Output
-- =============================================
CREATE PROCEDURE [dbo].[USP_APP_SEL_NOT_Macro]
    ...
    select @DiscrepancyTypeLkup = CMN_LookupMasterID  from CMN_LookupMaster with(nolock) where CMN_LookupTypeRef = 7 And LookupValue = @DiscrepancyType;
                     ...
                     Select GEN_QueueId as WorkItemID, 
                      ...
                      Where                                           
                            ...
                            AND (GQ.MostRecentWorkQueueLkup = 10009)  ---OOA - Open NOT 
							AND (GQ.IsNOTMacroProcessed = 0 or GQ.IsNOTMacroProcessed Is NULL)
END TRY 
